<script lang="ts">
  ...
  const readingTime = $derived(
    data.post.readingTime ||
      calculateReadingTime(data.post.content || "").minutes,
  );
  ...
  const schemaOrg = $derived({
    article: {
      ...
      image:
        data.post.ogImage || data.post.heroImage || `${SITE_URL}/favicon.svg`,
      datePublished: data.post.publishedAt || new Date().toISOString(),
      dateModified:
        data.post.updatedAt ||
        data.post.publishedAt ||
        new Date().toISOString(),
      // June 2026: Named Person author required for E-E-A-T — Organization author insufficient
      author: {
        "@type": "Person",
        "@id": `${SITE_URL}/about#author`,
        name: data.post.author || "Bill Kelly",
        url: `${SITE_URL}/about`,
...
<SEO
  ...
  description={data.post.seoDescription ||
    data.post.excerpt ||
    `Read ${data.post.title} on Trading Greats`}
...
<svelte:head>
  ...
  {#if data.post.category}
    <meta
      ...
      content={getCategoryLabel(data.post.category)}
    />
  {/if}
  {#if data.post.tags}
...
<div class="min-h-screen bg-midnight-950">
  ...
  <section class="relative overflow-hidden">
    ...
    {#if data.post.heroImage}
      <div
        class="absolute inset-0 h-[50vh] sm:h-[55vh] lg:h-[60vh] min-h-[350px] sm:min-h-[400px] lg:min-h-[500px]"
      >
        <img
        ...
        />
        <div
          class="absolute inset-0 bg-linear-to-t from-midnight-950 via-midnight-950/80 to-midnight-950/40"
        ></div>
      </div>
    {:else}
      <div
        class="absolute inset-0 h-[35vh] sm:h-[38vh] lg:h-[40vh] min-h-[280px] sm:min-h-[320px] lg:min-h-[400px]"
      >
        <div
          class="absolute inset-0 bg-linear-to-br from-midnight-900 via-midnight-950 to-midnight-900"
        ></div>
        <div
          class="absolute top-0 left-1/4 h-48 w-48 sm:h-72 sm:w-72 lg:h-96 lg:w-96 rounded-full bg-gold-500/5 blur-3xl"
        ></div>
        <div
          class="absolute bottom-0 right-1/4 h-48 w-48 sm:h-72 sm:w-72 lg:h-96 lg:w-96 rounded-full bg-gold-500/5 blur-3xl"
        ></div>
      </div>
    {/if}

    <div
      class="relative mx-auto max-w-4xl px-4 pt-20 sm:pt-24 lg:pt-32 pb-10 sm:pb-12 lg:pb-16 sm:px-6 lg:px-8"
    >
      <ScrollReveal>
        ...
        <!-- Meta - MOBILE-FIRST -->
        <div
          class="flex flex-wrap items-center gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-5 lg:mb-6"
        >
          {#if data.post.category}
            <a
          ...
          {#if data.post.publishedAt}
            <span
              class="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm text-midnight-400"
            >
        ...
        <!-- Title - MOBILE-FIRST -->
        <h1
          class="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight"
        >
          {data.post.title}
        </h1>
        ...
        {#if data.post.excerpt}
          <p
            class="mt-4 sm:mt-5 lg:mt-6 text-base sm:text-lg lg:text-xl text-midnight-300 leading-relaxed"
          >
            {data.post.excerpt}
          </p>
        {/if}
        ...
        <div
          class="mt-6 sm:mt-7 lg:mt-8 flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center justify-between gap-4 pt-6 sm:pt-7 lg:pt-8 border-t border-midnight-800/50"
        >
          {#if data.post.author}
            <div class="flex items-center gap-2.5 sm:gap-3">
              <div
                class="h-9 w-9 sm:h-10 sm:w-10 rounded-full bg-linear-to-br from-gold-400 to-gold-600 flex items-center justify-center"
              >
                <Icon
                  name="user"
                  class="h-4 w-4 sm:h-5 sm:w-5 text-midnight-950"
                />
              </div>
              <div>
    ...
    <div class="mx-auto max-w-4xl px-4 py-8 sm:py-10 lg:py-12 sm:px-6 lg:px-8">
      <ScrollReveal>
        ...
        {#if data.post.heroImage && data.post.heroImageCaption}
          <p
            class="mb-8 sm:mb-10 lg:mb-12 text-center text-xs sm:text-sm text-midnight-500 italic"
          >
        ...
        {#if data.post.tags && data.post.tags.length > 0}
          <div
            class="mt-8 sm:mt-10 lg:mt-12 pt-6 sm:pt-7 lg:pt-8 border-t border-midnight-800/50"
          >
            <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
              <Icon
                name="tag"
                class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-midnight-500"
              />
              {#each data.post.tags as tag (tag)}
                <span
                  class="rounded-full bg-midnight-800 px-2.5 sm:px-3 py-0.5 sm:py-1 text-xs sm:text-sm text-midnight-300"
                >
                  {tag}
                </span>
              {/each}
        ...
        <div
          class="mt-8 sm:mt-10 lg:mt-12 pt-6 sm:pt-8 border-t border-midnight-800/50"
        >
          <div class="flex items-center gap-3 sm:gap-4">
            <div
              class="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-full bg-gold-500/10 ring-1 ring-gold-500/30"
            >
              <Icon
                name="trending-up"
                class="h-5 w-5 sm:h-6 sm:w-6 text-gold-400"
              />
            </div>
            <div>
              ...
              <p class="text-xs sm:text-sm text-midnight-400">
                Published by <a
                  ...
                  >Trading Greats</a
                > — research-driven profiles of legendary traders and their strategies.
              </p>
            </div>
          </div>
        </div>
  ...
  {#if data.relatedPosts.length > 0}
    <section class="border-t border-midnight-800/50 bg-midnight-900/30">
      <div
        class="mx-auto max-w-7xl 2xl:max-w-[1440px] 3xl:max-w-[1680px] 4xl:max-w-[2000px] px-4 py-10 sm:py-12 lg:py-16 sm:px-6 lg:px-8"
      >
        <ScrollReveal>
          <h2
            class="font-display text-xl sm:text-2xl font-bold text-white mb-5 sm:mb-6 lg:mb-8"
          >
            Continue Reading
          </h2>

          <div
            class="grid gap-4 sm:gap-6 lg:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-5"
          >
            {#each data.relatedPosts as post, i (post.slug)}
              <ScrollReveal delay={i * 75}>
                ...
                >
                  {#if post.heroImage}
                    <div
                      class="relative aspect-video sm:aspect-16/10 overflow-hidden"
                    >
                      <img
                        src={post.heroImage}
                      ...
                      />
                      <div
                        class="absolute inset-0 bg-linear-to-t from-midnight-950/60 to-transparent"
                      ></div>
                    </div>
                  {:else}
                    <div
                      class="relative aspect-video sm:aspect-16/10 bg-midnight-800 flex items-center justify-center"
                    >
                      <Icon
                        name="book-open"
                        class="h-10 w-10 sm:h-12 sm:w-12 text-midnight-600"
                      />
                    </div>
                  {/if}
                  ...
                  <div class="flex flex-1 flex-col p-4 sm:p-5 lg:p-6">
                    <h3
                      class="font-display text-base sm:text-lg font-bold text-white group-hover:text-gold-400 transition-colors line-clamp-2"
                    >
                      {post.title}
                    </h3>

                    <div
                      class="mt-3 sm:mt-4 flex items-center justify-between text-[10px] sm:text-xs text-midnight-500"
                    >
                      ...
                      <Icon
                        name="arrow-right"
                        class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-gold-500 opacity-100 sm:opacity-0 sm:-translate-x-2 sm:group-hover:opacity-100 sm:group-hover:translate-x-0 transition-all"
                      />
                    </div>
                  </div>
                </a>
  ...
  <section class="border-t border-midnight-800/50">
    <div
      class="mx-auto max-w-7xl 2xl:max-w-[1440px] 3xl:max-w-[1680px] 4xl:max-w-[2000px] px-4 py-8 sm:py-10 lg:py-12 sm:px-6 lg:px-8"
    >
